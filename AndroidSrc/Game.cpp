...
public:
    ...
    void Start()
    {
		...
		{
			// На мобильных платформах инициализируем Screen touch
			InitTouchInput();
		}
    ...
    void HandleBeginFrame(StringHash eventType, VariantMap& eventData)
    {
        ...
        else if (GLOBAL->neededGameState_ == GS_INTRO)
        {
            ...
            {
                Vector3 pos = barriers[i]->GetPosition();
                pos.y_ = BAR_RANDOM_Y;
    ...
    void UpdateUIVisibility()
    {
        ...
        const Vector<SharedPtr<UIElement > > & uiElements = UI_ROOT->GetChildren();
        for (Vector<SharedPtr<UIElement > >::ConstIterator i = uiElements.Begin(); i != uiElements.End(); i++)
        {
            UIElement* element = *i;
    ...
    void CreateUI()
    {
        ...
        helpText->SetText(L"Нажмите на экран");
    }
    ...
    void HandleUpdate(StringHash eventType, VariantMap& eventData)
    {
		...
		if (touchEnabled_)
		{
			for (unsigned i = 0; i < INPUT->GetNumTouches(); ++i)
			{
				...
				if (!state->touchedElement_)    // Если нажато на пустое место
				{
					ChangeState();
				}
			}
